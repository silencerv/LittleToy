...
public class AVLTree<K extends Comparable, V> {
    ...
    private TreeNode<K, V> insert(TreeNode currentNode, K key, V value) {
        ...
        if (comResult > 0) {
            ...
            if (balanceFac == -2) {
                if (key.compareTo(currentNode.right.key) < 0) //RL旋转
            ...
            int balanceFac = getHeight(currentNode.left) - getHeight(currentNode.right);
            if (balanceFac == 2) {
                if (key.compareTo(currentNode.left.key) > 0) //LR旋转
    ...
    //左右旋
    private TreeNode leftRightRotation(TreeNode currentNode) {
        currentNode.left = leftRotation(currentNode.left);
        return rightRotation(currentNode);
    }

    //右左旋
    private TreeNode rightLeftRotation(TreeNode currentNode) {
