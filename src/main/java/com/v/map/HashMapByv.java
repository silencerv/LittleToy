...
/**
 * Created by v on 2016/10/7.
 */
public class HashMapByv<K,V> extends AbstractMap<K,V> implements Map<K,V> {
    ...
    public HashMapByv(int capacity, float factor) {
    ...
    private V putVal(int hash,K key,V value,boolean absent){
        ...
        if (table == null || (size + 1) > threshold)
            resize();
        //首先判断是否有hash冲突
        oldElement = table[(table.length - 1 ) & hash];
